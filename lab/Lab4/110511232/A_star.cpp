...
{
    // manhattan distance
    double dist_cost =  (abs(src_idx.first - tar_idx.first) + abs(src_idx.second - tar_idx.second)) * Alpha;
    // overflow cost
    // sum up the overflow cost of the neighbors of the src grid
    double ov_cost = 0;
    // left neighbor ov
    if(src_idx.second > 0)
    {
        ov_cost += grid[src_idx.first][src_idx.second].leftHorCap <= grid[src_idx.first][src_idx.second].horNetCount;
    }

    // right neighbor ov
    if(src_idx.second < GRID_DIM_HOR-1)
    {
        ov_cost += grid[src_idx.first][src_idx.second+1].leftHorCap <= grid[src_idx.first][src_idx.second+1].horNetCount;
    }

    // up neighbor ov
    if(src_idx.first < GRID_DIM_VER-1)
    {
        ov_cost += grid[src_idx.first+1][src_idx.second].bottomVerCap <= grid[src_idx.first+1][src_idx.second].verNetCount;
    }

    // down neighbor ov
    if(src_idx.first > 0)
    {
        ov_cost += grid[src_idx.first][src_idx.second].bottomVerCap <= grid[src_idx.first][src_idx.second].verNetCount;
    }
    ov_cost *= Beta * 0.5 * MetalCost_max;

    // via cost
    double via_cost = Delta * ViaCost;
    // GCell cost
    double gcell_cost = (abs(src_idx.first - tar_idx.first)*M2Cost_max + abs(src_idx.second - tar_idx.second)*M1Cost_max) * Gamma;

    return dist_cost + ov_cost + via_cost + gcell_cost;
}
